...
#                                                                             
# PROGRAMMER: AHMET CELIK 
# DATE CREATED: 09 APRIL 2020
# REVISED DATE: 
    ...
    for key in results_stats_dic:
        ...
        if my_list[0] == "pct":
            ...
            for i in range(1,len(my_list)):
                perc_title += " "
                perc_title += my_list[i]
            perc_title = perc_title.title()
            print(perc_title + ": " + str(results_stats_dic[key]))
            
    print("")
    if print_incorrect_dogs:
        if results_stats_dic["n_correct_dogs"] + results_stats_dic["n_correct_notdogs"] == results_stats_dic["n_images"]:
            print("There are no Misclassified Dogs")
        else:
            print("Misclassified Dogs:")
            for key in results_dic:
                if sum(results_dic[key][3:]) == 1:
                    print("Pet Image: {} and Classifier Label: {}".format(key, results_dic[key][1]))
                    
    print("")
    if print_incorrect_breed:
        if results_stats_dic["n_correct_dogs"] == results_stats_dic["n_correct_breed"]:
            print("There are no Misclassified Breed\'s of Dog")
        else:
